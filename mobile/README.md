...
build **stable** (`better-xcloud.user.js`, v1.13.6) dès le début de la page.
...
`evenbetter-xcloud-1.13.6.apk` (~1,1 Mo) — package `com.bxperf.app`, signé avec
...
bash build.sh
# → out/evenbetter-xcloud-1.13.6.apk
```
...
| APK | `evenbetter-xcloud-1.13.6.apk` | `evenbetter-xcloud-1.13.6-preview1.apk` |

```bash
bash build.sh                    # stable
VARIANT=preview bash build.sh    # preview → out/evenbetter-xcloud-1.13.6-preview1.apk
```
