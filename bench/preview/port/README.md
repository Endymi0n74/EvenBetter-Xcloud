...
| `t10-auto-spoof.test.js` | **Gate CI de la logique T10** (`bench/preview/port/t10-auto-spoof.test.js`, branché au step preview de bench.yml) : extraction du statement T10 du build preview + pureté du stable (guard BX_PREVIEW) + comportement vm sur mock UserAgent (Firefox/Safari → spoof, Edge/Chrome/Crios → inchangé, profil explicite jamais écrasé, BX_PREVIEW=false → inchangé) — 13 checks, `--self-test` rejoue le chemin d'échec sur copies corrompues (build réel intact) |
